...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if ref_id:
        message += f\" Вы приглашены партнером ID: {ref_id}\"
...
async def purchase_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(f\"✅ Закупка на {amount} руб. внесена! Комиссии распределены.\")
...
async def profile_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    with get_session() as session:
        ...
        msg = (
            f\"👤 *Ваш профиль* \"
            f\"🆔 ID: `{user.id}` \"
            f\"📊 Статус: {'✅ Активен' if partner.status.value == 'активный' else '❌ Неактивен'} \"
